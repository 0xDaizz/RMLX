...
| 5A | NN Inference Core | Qwen, DeepSeek, Mixtral, Kimi | Phase 4 | Complete |
...
- `rmlx-nn`: Model architectures (Qwen 3.5, DeepSeek-V3, Mixtral, Kimi K2.5)
...
### Benchmark Results (M3 Ultra, f16, MoE expert shapes)
...
| **rmlx-nn** | 11 new activations (16 total: +ReLU, LeakyReLU, ELU, SELU, Mish, QuickGELU, HardSwish, HardSigmoid, Softplus, Softsign, GLU); full MLA forward (DeepSeek-V3 9-step pipeline with latent KV compression); full SlidingWindowAttention forward (Mistral-style RoPE+SDPA+KV cache); AwqLinear/GptqLinear/KQuantType/KQuantConfig; K-quant GGUF mapping; radix-tree prefix cache with LRU eviction; chunked prefill scheduler; 3 full model architectures (Qwen2Model, DeepSeekV3Model, MixtralModel) | Complete |
