...
rmlx 프로젝트의 7단계 구현 로드맵입니다. 각 Phase는 이전 Phase의 산출물에 의존하며, 순차적으로 진행됩니다.
...
| 1 | Zero-Copy + RDMA | ZeroCopyBuffer, DualRegPool, ibverbs FFI, blocking_exchange | Phase 0 | 대기 |
| 2 | Metal Compute | Array 타입, Metal 커널 10종 (matmul, softmax, rope 등) | Phase 0 | 대기 |
| 3 | Pipeline Overlap | MTLSharedEvent, dual-queue pipeline | Phase 2 | 대기 |
| 4 | Expert Parallelism | EP dispatch/combine, 3-zone auto backend, sparse dispatch | Phase 1 + 3 | 대기 |
| 5 | Model Serving | rmlx-lm (별도 리포) | Phase 4 | 대기 |
| 6 | Multi-Port | 듀얼 TB5 multi-port striping | Phase 4 | 대기 |
| 7 | Production | Hardening, VJP, PyO3 바인딩 | Phase 5 | 대기 |
...
graph LR
    ...
    style P7 fill:#94a3b8,color:#fff
```
...
- [ ] `test_zero_copy_buffer_lifecycle` -- InFlightToken drop 후 free 검증
- [ ] `test_dual_registration` -- Metal + ibv_mr 동일 주소 검증
- [ ] `test_rdma_exchange_2node` -- 4MB 라운드트립, 0 mismatch
- [ ] `test_rdma_startup_probe` -- GID/MR/QP 런타임 탐색 성공
- [ ] `test_recv_before_send_invariant` -- recv 미포스팅 시 에러 반환
- [ ] 벤치마크: RDMA 대역폭 > 6 GB/s (단일 포트)
- [ ] Codex 리뷰: FFI 경계 안전성, lifetime 검증

---

## Phase 2: Metal Compute (4주)
...
- [ ] `cargo fmt --all --check` -- diff 0
- [ ] `cargo clippy --workspace -- -D warnings` -- 0 warnings
- [ ] 10종 커널 각각 MLX 대비 +/-5% 성능
- [ ] `test_matmul_correctness` -- fp16/bf16 정합성 (ulp < 2)
- [ ] `test_quantized_matmul` -- q4/q8 정합성
- [ ] `test_dispatch_geometry` -- threadgroup vs thread 크기 검증
- [ ] Codex 리뷰: 커널 바인딩 인덱스 일치 확인

---

## Phase 3: Pipeline Overlap (3주)
...
- [ ] `cargo fmt --all --check` -- diff 0
- [ ] `cargo clippy --workspace -- -D warnings` -- 0 warnings
- [ ] `test_shared_event_latency` -- spin-wait < 280us
- [ ] `test_dual_queue_overlap` -- 두 큐 동시 실행 확인
- [ ] `test_layer_pipeline_correctness` -- 파이프라인 결과 == 직렬 결과
- [ ] `test_event_deadline_cancel` -- 타임아웃/취소 동작 확인
- [ ] 벤치마크: 동기화 레이턴시 히스토그램 (p50/p95/p99)
- [ ] Codex 리뷰: 동기화 프로토콜 정합성

---

## Phase 4: Expert Parallelism (4주)
...
- [ ] `cargo fmt --all --check` -- diff 0
- [ ] `cargo clippy --workspace -- -D warnings` -- 0 warnings
- [ ] `test_1rank_vs_2rank_parity` -- 단일 노드 결과 == 2-node EP 결과
- [ ] `test_3zone_policy` -- N=1/64/256/1024 각각 올바른 backend 선택
- [ ] `test_sparse_dispatch_correctness` -- matmul scatter == dense 결과
- [ ] `test_interleaved_exchange_stress` -- 1000 연속 교환 0 에러
- [ ] `test_capacity_overflow_detection` -- overflow_count 메트릭 정확성
- [ ] 벤치마크: 2-node decode step < 35ms
- [ ] Codex 리뷰: exchange 프로토콜, 메트릭 수집 정확성

---

## Phase 5: Model Serving (4주)
...
- [ ] `cargo fmt --all --check` -- diff 0 (양쪽 리포지토리)
- [ ] `cargo clippy --workspace -- -D warnings` -- 0 warnings (양쪽)
- [ ] `test_e2e_single_token_generation` -- 단일 토큰 생성 정합성
- [ ] `test_continuous_batching` -- 다중 요청 동시 처리
- [ ] `test_kv_cache_paged` -- 캐시 할당/해제 stress
- [ ] `test_openai_api_compat` -- /v1/chat/completions 응답 포맷
- [ ] `test_metrics_prometheus` -- /metrics 엔드포인트 파싱 성공
- [ ] 벤치마크: 2-node EP decode > 28 tok/s
- [ ] Codex 리뷰: 서빙 안정성, 메모리 누수

---

## Phase 6: Multi-Port (3주)
...
- [ ] `cargo fmt --all --check` -- diff 0
- [ ] `cargo clippy --workspace -- -D warnings` -- 0 warnings
- [ ] `test_dual_port_striping` -- 2포트 동시 전송, 데이터 정합성
- [ ] `test_single_port_fallback` -- 1포트 실패 시 graceful fallback
- [ ] 벤치마크: 듀얼 포트 대역폭 > 12 GB/s
- [ ] Codex 리뷰: 포트 간 독립성, 에러 격리

---

## Phase 7: Production (장기)

### 목표

프로덕션 안정성, 관찰성, 그리고 향후 학습 지원을 위한 기반을 구축합니다. PyO3 바인딩을 통해 Python 생태계와의 호환성을 확보합니다.
...
- PyO3 Python 바인딩 (`pip install rmlx`)

### 완료 조건 (DoD)

- [ ] Structured logging 전체 크레이트 적용
- [ ] Prometheus /metrics 엔드포인트 동작 확인
- [ ] Graceful shutdown 시나리오 테스트
- [ ] VJP 기본 연산 (matmul, softmax) gradient 정합성
- [ ] PyO3 바인딩: `import rmlx` 후 기본 Array 연산 동작
- [ ] maturin 빌드: `pip install rmlx` 원클릭 설치 성공
