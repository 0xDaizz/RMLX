...
rmlx 프로젝트의 구현 로드맵입니다. Phase 9B-opt 및 서빙 지원 Phase S1-S5, 그리고 전체 크레이트 감사 수정(76개 항목)까지 모든 Phase가 완료되었습니다. 현재 테스트 수: 1,142+. Phase 8c는 CachedDecode (사전 해석 PSO + 사전 할당 스크래치 버퍼), 2-인코더 디코드 경로, `_preresolved_into_encoder` 패턴, GEMV BM8 최적화 (배리어 제거 + f16 확대 로드)를 추가하여 60L 깊이에서 714 us/layer를 달성합니다 (f16, 6x 낮은 분산). Phase 10 (커널 융합)은 fused_rms_gemv와 fused_swiglu_down 커널을 추가하여 디코드 파이프라인을 9에서 7 디스패치로 줄이고, 703.4 us/layer를 달성합니다. Phase 11 (GEMV 커널 최적화 실험)은 모든 커널 수준 최적화 시도가 실패했음을 확인했습니다 (col-major +84%, interleaved +2.2%, SRAM+f16+funcconst +3.6%); row-major BM8 GEMV + f32 누산기가 705 us/layer의 실질적 하한선입니다 (73.6% 대역폭 효율). 현재 테스트 수: 1,151.
