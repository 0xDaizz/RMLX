...
rmlx 프로젝트의 구현 로드맵입니다. Phase 9B-opt 및 서빙 지원 Phase S1-S5, 그리고 전체 크레이트 감사 수정(76개 항목)까지 모든 Phase가 완료되었습니다. 현재 테스트 수: 543.
...
| EP-1: GPU-Native Top-K 라우팅 (`topk_route.rs`) | main (merged) | 543+ tests | ✅ Complete |
| EP-2: 그룹형 Expert GEMM + 가중치 스태킹 (`expert_group.rs`, `gather_mm.rs`) | main (merged) | 543+ tests | ✅ Complete |
| EP-3: 가변 길이 v3 프로토콜 (`v3_protocol.rs`) | main (merged) | 543+ tests | ✅ Complete |
| EP-4: Compute-Communication 오버랩 (TBO + SBO) (`moe_pipeline.rs`) | main (merged) | 543+ tests | ✅ Complete |
| EP-5: FP8 와이어 포맷 (`fp8.rs`, `fp8_exchange.rs`) | main (merged) | 543+ tests | ✅ Complete |
| EP-6: ICB Sparse Expert 실행 + RDMA Slab 링 (`icb_sparse.rs`, `slab_ring.rs`) | main (merged) | 543+ tests | ✅ Complete |
