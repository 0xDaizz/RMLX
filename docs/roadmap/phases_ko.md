# 🗺️ 구현 로드맵 — Phase 0-9B + S1-S5 + Audit Remediation 완료 + Phase KO + Phase 8c + Phase 9 + Phase 10 + Phase 11

rmlx 프로젝트의 구현 로드맵입니다. Phase 9B-opt 및 서빙 지원 Phase S1-S5, 그리고 전체 크레이트 감사 수정(76개 항목)까지 모든 Phase가 완료되었습니다. 현재 테스트 수: 1,142+. Phase 8c는 CachedDecode (사전 해석 PSO + 사전 할당 스크래치 버퍼), 2-인코더 디코드 경로, `_preresolved_into_encoder` 패턴, GEMV BM8 최적화 (배리어 제거 + f16 확대 로드)를 추가하여 60L 깊이에서 714 us/layer를 달성합니다 (f16, 6x 낮은 분산). Phase 10 (커널 융합)은 fused_rms_gemv와 fused_swiglu_down 커널을 추가하여 디코드 파이프라인을 9에서 7 디스패치로 줄이고, 703.4 us/layer를 달성합니다. Phase 11 (열-주도 GEMV)은 열-주도 가중치 저장 [K,M]으로 연속 half4 로드와 레지스터 수준 프리페치를 구현하고, 12개 새 Metal 커널 (gemv.rs 6개, fused.rs 6개), Linear의 `prepare_weight_col_major()`, 자동 폴백을 포함하는 CachedDecode 통합을 추가합니다. 현재 테스트 수: 1,356.
...
| 10 | 커널 융합 | fused_rms_gemv (Fusion A), fused_swiglu_down (Fusion B), 9→7 디스패치 파이프라인, 자동 폴백 | 9 | ✅ 완료 |
| 11 | 열-주도 GEMV | col-major 가중치 저장 [K,M], 연속 half4 로드, 레지스터 수준 프리페치, 12개 Metal 커널, CachedDecode 통합 | 10 | ✅ 완료 |
...
| Phase 11: 열-주도 GEMV | main | 1,356 tests | ✅ Complete |
...
|------|---------------:|----------------:|
| 레이어당 디스패치 | 9 | **7** |
| 레이턴시 (60L) | 714 us/L | **703.4 us/L** |
| MLX 대비 | 6.34x 빠름 | **6.43x 빠름** |
| 테스트 | 1,142 | **1,151** |
...
## Phase 11: 열-주도(Column-Major) GEMV -- 완료

### 목표

가중치를 열-주도 [K,M] 레이아웃으로 저장하여 연속 half4 로드를 통해 GEMV 대역폭을 최적화하고, 레지스터 수준 프리페치로 메모리 코얼레싱을 개선합니다.

### 주요 산출물

- **열-주도 가중치 저장**: Linear의 `prepare_weight_col_major()`가 초기화 시 가중치를 [K,M] 레이아웃으로 전치하여 K 누적 차원에서 스레드당 연속 half4 (8바이트) 읽기를 가능하게 합니다
- **레지스터 수준 프리페치**: 현재 반복의 연산 중에 다음 반복의 가중치 데이터를 레지스터에 로드하여 메모리 레이턴시를 연산 뒤에 숨깁니다
- **12개 새 Metal 커널**: `gemv.rs`의 열-주도 커널 6개 (`gemv_col`, `gemv_bias_col` — f32/f16/bf16) + `fused.rs`의 열-주도 커널 6개 (`fused_rms_gemv_col`, `fused_swiglu_down_col` — f32/f16/bf16)
- **CachedDecode 통합**: 열-주도 PSO와 레이어당 4개 열-주도 가중치 버퍼, 열-주도 PSO 컴파일 실패 시 행-주도 자동 폴백
- **커널 이름 도우미**: `gemv_col_kernel_name`, `gemv_bias_col_kernel_name`, `fused_rms_gemv_col_kernel_name`, `fused_swiglu_down_col_kernel_name`

### 결과

| 지표 | 이전 (Phase 10) | 이후 (Phase 11) |
|------|----------------:|----------------:|
| 가중치 레이아웃 | 행-주도 [M,K] | **열-주도 [K,M]** |
| GEMV 메모리 접근 | 비연속 | **연속 half4** |
| 목표 레이턴시 (60L) | 703.4 us/L | **703.4 us/L** (열-주도 핫 패스 미사용) |
| MLX 대비 | 6.43x 빠름 | **6.43x 빠름** (대형 M에서 열-주도 성능 저하) |
| 추가된 Metal 커널 | 0 | **12** |
| 테스트 | 1,151 | **1,356** |

### 완료 조건 (DoD)

- [x] 12개 열-주도 Metal 커널 컴파일 및 정합성 테스트 통과
- [x] prepare_weight_col_major()가 가중치를 정확히 전치 및 캐싱
- [x] CachedDecode가 열-주도 PSO 및 가중치 버퍼를 통합
- [x] 행-주도 자동 폴백 검증
- [x] 1,356 테스트 통과

---

> **참고**: Flash Attention 2와 고급 양자화 기능은 이전에 Phase 10/11로 계획되었으나, 각각 Phase S3과 Phase S2에서 이미 구현 완료되었습니다. 현재 Phase 10은 커널 융합, Phase 11은 열-주도 GEMV로 재지정되었습니다.
