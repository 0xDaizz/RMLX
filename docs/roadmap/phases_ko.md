...
rmlx 프로젝트의 구현 로드맵입니다. Phase 9B-opt 및 서빙 지원 Phase S1-S5, 그리고 전체 크레이트 감사 수정(76개 항목)까지 모든 Phase가 완료되었습니다. 현재 테스트 수: 1,298+. Phase 8c는 CachedDecode (사전 해석 PSO + 사전 할당 스크래치 버퍼), 2-인코더 디코드 경로, `_preresolved_into_encoder` 패턴, GEMV BM8 최적화 (배리어 제거 + f16 확대 로드)를 추가하여 60L 깊이에서 714 us/layer를 달성합니다 (f16, 6x 낮은 분산). Phase 10 (커널 융합)은 fused_rms_gemv와 fused_swiglu_down 커널을 추가하여 디코드 파이프라인을 9에서 7 디스패치로 줄이고, 699.3 us/layer를 달성합니다. Phase 11 (GEMV 커널 최적화 실험)은 모든 커널 수준 최적화 시도가 실패했음을 확인했습니다 (col-major +84%, interleaved +2.2%, SRAM+f16+funcconst +3.6%); row-major BM8 GEMV + f32 누산기가 705 us/layer의 실질적 하한선입니다 (73.6% 대역폭 효율). Phase A (프리필 최적화)는 단일-CB 프리필 파이프라인(54개 동기화 지점→1), GQA slab SDPA(32개 디스패치→1), GEMM threadgroup swizzle, 새 연산 matmul_into_cb/silu_into_cb를 추가하여 베이스라인 대비 3.5-7.3x 속도 향상, MLX 대비 1.2-3.4x 이내를 달성합니다. Phase B (GEMM Config Sweep)는 3회 sweep에서 27개 커널 변형을 테스트하여 bk32_2x4 (BM=64, BN=64, BK=32, SG=2x4, 256 스레드)를 최적 구성으로 확정했습니다 — 21.54T TFLOPS, MLX 23.97T 대비 -10.1% 격차. Phase C (커널 수준 최적화)는 wide_load와 SG=2x4 레이아웃을 프로덕션 커널에 적용하여 21.21T를 달성했습니다. Phase D2 (MLX 아키텍처 커널)는 BK=16, 2 SG, 64 스레드, 단일 버퍼, 4xhalf4 와이드 로드로 **24.05T TFLOPS**를 달성하여 MLX 패리티를 초과 달성했습니다 (+0.3%). PR#77-79는 프리필 SDPA 패리티(QMM Q4 M=512: 17.43T, MLX 대비 +28%), NAX GEMM, Rust 디스패치 오버헤드를 추가 최적화하여 FP16 GEMM 24.05T, 디코드 699.3 us/layer를 달성했습니다. PR#80은 투기적 디코딩 프레임워크 프리미티브를 추가했습니다. objc2-metal 마이그레이션(M-1)은 metal-rs를 objc2-metal 생태계로 교체하고, Metal 4 API 통합(M-2)은 feature-gated Metal 4 지원을 추가했습니다. Phase F (인프라 최적화)는 디스패치 오버헤드 벤치마크(176us/CB, 12.4%), DiskPipelineCache, GatherMM MMA(4-12x)를 추가했습니다. Phase G (양자화 최적화)는 QMM MMA Q4/Q8, QMV qdot, CPU 폴백 제거를 완료했습니다. Phase H-2 (GEMM+잔차 에필로그 퓨전)는 대형 N에서 5-12% 개선을 달성했습니다. Phase I-1 (분산 TP)은 DistributedTransformerModel과 forward_with_group/shard_for_tp를 추가하여 TP=2에서 1.94x 추정 속도 향상을 달성했습니다. Phase J (양자화 패리티 + 인프라)는 QMM 격차를 4.78x에서 2.55x로 축소(+73%), QMV 격차를 1.15x로 축소(+37%), ExecGraph inter-layer stall 32개 제거, FusionCompiler(lazy.rs -> FusionAnalyzer -> codegen -> Metal JIT) 추가, RMSNorm+GEMM fusion(function constant 203, -40.5%), Split-K QMM, MoE 융합 커널, forward_auto()(eager+lazy 하이브리드 디스패치)를 추가했습니다.
...
|:-----:|------|----------|:---------:|:----:|
| 0 | 스캐폴딩 | Workspace, objc2-metal 래퍼, CI | -- | ✅ 완료 |
...
| 10 | 커널 융합 | fused_rms_gemv (Fusion A), fused_swiglu_down (Fusion B), 9→7 디스패치 파이프라인, 자동 폴백 — **699.3 us/layer** | 9 | ✅ 완료 |
...
| D | GEMM 커널 아키텍처 | D2: MLX 아키텍처 커널 — **24.05T** (MLX 대비 +0.3%), D3: function constants, D5: bf16 배리어 수정 | C | D2 완료 |
...
| PR#77 | 프리필 SDPA 패리티 | Q4 QMM/QMV 94% MLX 처리량, QMM Q4 M=512 **17.43T** (+28% vs MLX) | J | ✅ 완료 |
| PR#78 | NAX GEMM + SDPA 4D | NAX 64x128 GEMM, SDPA strided 4D, 프리필 디스패치 12→9 | PR#77 | ✅ 완료 |
| PR#79 | Rust 디스패치 최적화 | P0-P6 디스패치 최적화, ArrayPool, 융합 norm 폴백, FP16 GEMM **24.05T** | PR#78 | ✅ 완료 |
| PR#80 | 투기적 디코딩 프레임워크 | Draft/verify/accept 프레임워크, 트리 어텐션, 다중 후보 점수화 | PR#79 | ✅ 완료 |
| M-1 | objc2-metal 마이그레이션 | rmlx-metal을 metal-rs에서 objc2-metal 생태계로 마이그레이션, unsafe/msg_send 최소화 | PR#80 | ✅ 완료 |
| M-2 | Metal 4 API 통합 | Feature-gated Metal 4 API (`metal4` 피처 플래그), 메시 셰이더, 레이트레이싱 스텁 | M-1 | ✅ 완료 |
...
| Phase D2: MLX 아키텍처 커널 (24.05T, +0.3% vs MLX) | gemm-kernel-d2 | 1,298+ tests | ✅ Complete |
...
| PR#77: 프리필 SDPA 패리티 (QMM Q4 17.43T, +28% vs MLX) | main (PR #77) | 1,298+ tests | ✅ Complete |
| PR#78: NAX GEMM + SDPA strided 4D + 프리필 디스패치 12→9 | main (PR #78) | 1,298+ tests | ✅ Complete |
| PR#79: Rust 디스패치 최적화 P0-P6 + ArrayPool (FP16 GEMM 24.05T) | main (PR #79) | 1,298+ tests | ✅ Complete |
| PR#80: 투기적 디코딩 프레임워크 프리미티브 | main (PR #80) | 1,298+ tests | ✅ Complete |
| M-1: objc2-metal 마이그레이션 (metal-rs → objc2-metal 생태계) | feat/objc2-metal-migration | 1,298+ tests | ✅ Complete |
| M-2: Metal 4 API 통합 (feature-gated) | feat/objc2-metal-migration | 1,298+ tests | ✅ Complete |
...
Cargo workspace 구조를 확립하고, objc2-metal 기본 동작을 검증하며, CI를 설정합니다.
...
|------|---------------:|----------------:|
| 레이어당 디스패치 | 9 | **7** |
| 실측 레이턴시 (60L) | 714 us/L | **699.3 us/L** (M3 Ultra, f16, 60L) |
| 테스트 | 1,142 | **1,151** |
...
## Phase D: GEMM 커널 아키텍처 — 진행 중

### 목표

커널 아키텍처 변경(점유율, 저장 경로, 정렬 특수화)을 통해 MLX 대비 잔여 ~11.5% GEMM 처리량 격차를 해소합니다.

### 세부 단계

#### D1: HiPerf 커널 활성화 — 실패

기존 `gemm_hiperf_f16` 커널(BK=16, 4 SG, 128 스레드, 이중 버퍼)을 활성화했습니다. 결과: **-39% 성능 저하** (12.88T vs 21.21T). 근본 원인:

1. **스칼라 로드**: transaction당 2 bytes vs Full 커널의 16 bytes(2×half4) — 8배 비효율
2. **2배 K 반복**: BK=16이 BK=32 대비 루프 횟수와 배리어 오버헤드를 2배로 증가
3. **2배 저장 배리어**: acc[4][4] = 16 저장 라운드(32 배리어) vs Full의 acc[4][2] = 8 라운드(16 배리어)

3배 점유율 향상이 이러한 비효율을 상쇄하지 못했습니다. HiPerf 커널은 최적화되지 않은 dead code였습니다. **되돌림.**

#### D2: MLX 아키텍처 커널 — 진행 중

MLX의 M3 레거시 아키텍처를 따르는 새 커널 작성: BK=16, 2 SG (WM=1, WN=2), 64 스레드, 단일 버퍼, ReadVector 스타일 와이드 로드(4×half4 = 스레드당 16 원소), 레지스터→디바이스 직접 저장(scratch 버퍼 제거), 서펜타인 MMA 순서.

- **예상 효과**: +15-25% (5-6 TG/core, 저장 배리어 0개, 와이드 로드)
- **난이도**: 높음 (새 커널 설계, gemm_kernel_opt.rs에서 벤치마크 먼저)

#### D3: Function Constants를 통한 정렬 특수화

런타임 분기 대신 Metal function constants(`align_M`, `align_N`, `align_K`)를 사용하여 bounds check를 파이프라인 생성 시점에 컴파일러가 dead code를 제거합니다.

- **예상 효과**: 정렬된 차원에서 +2-5%
- **난이도**: 낮음-중간 (JIT 파이프라인 수정)

#### D4: 저장 경로 배치 처리 (Full 커널)

8개 누적기 타일을 단일 scratch→device 쓰기 패스로 배치하여 저장 단계 배리어를 16개에서 2개로 축소합니다.
...
bf16 커널의 fragment당 배리어 폭발(bf16→f32 변환으로 K 루프 반복당 24개 추가 배리어)을 수정합니다. 변환을 fragment 로드 시점으로 이동합니다.
...
- [x] ~~D1: HiPerf 커널 활성화~~ — 실패(-39% 성능 저하), 되돌림
- [ ] D2: MLX 아키텍처 커널 벤치마크, 우수할 경우 프로덕션 적용
...
### 벤치마크 결과 (node0, M3 Ultra 80-core)
