...
Phase C는 Phase B config sweep 결과를 기반으로 GEMM 커널에 커널 수준 최적화를 적용합니다. 6-variant 절삭 벤치마크(`gemm_kernel_opt.rs`)를 통해 3가지 최적화를 테스트했습니다:

| 최적화 | 설명 | 결과 |
|--------|------|------|
| `direct_store` | simdgroup 레지스터에서 device 메모리로 직접 쓰기 (scratch 버퍼 제거) | -1-2% (비코얼레싱 scatter) |
| `wide_load` | 반복당 2×half4 (4개 대신 8개 원소) | **+5% (적용됨)** |
| `aligned` | 타일 정렬 차원에 대한 bounds check 제거 | 대형 행렬에서 성능 붕괴 |

추가로, SG=2×4 레이아웃(Phase B에서 최적 확인)이 프로덕션 matmul.rs 커널(f32/f16/bf16)에 적용되었고, `gemm_bench.rs`가 사전 할당 버퍼를 사용한 직접 커널 디스패치로 수정되었습니다.

| 지표 | 이전 | 이후 |
|------|-----:|-----:|
| GEMM TFLOPS (M=2048, N=14336) | 15.73T | 24.05T |
| 개선 | — | +52.9% |
| MLX(23.97T) 대비 | — | **+0.3% (파이프 패리티)** |

### GEMM 디스패치 테이블 (f16 — `matmul_into_cb` 프로덕션 경로)

| M | 디스패치 | 비고 |
|---|---------|------|
| 1 | Tiled GEMM (BM=16 pad) | GEMV 경로 제거됨 |
| 2-8 | Split-K (무조건, K>=256) | 작은 M에서 항상 Split-K |
| 9-32 | Split-K (K>=N만), else Tiled | 조건부 Split-K |
| 33-128 | Tiled GEMM (MlxMicro 16x32) | 표준 타일 경로 |
| 256 | MlxSmall 32x32 | 중간 타일 |
| 512+ | NAX 64x128 | 대형 M 최적화 |

### 성능 요약

| 지표 | 값 | 비고 |
|------|---:|------|
| FP16 GEMM | 24.05T | MLX 파이프 패리티 |
| QMM Q4 M=512 | 17.43T | MLX 13.6T, +28% |
| 디코드 최적 | 699.3 us/layer | Phase 10, 7-디스패치 융합 |
| BW 효율 | 73.6% | Apple Silicon f16 실질적 한계 |
...
pub struct KernelRegistry {
    ...
    jit_cache: Mutex<HashMap<String, MtlLibrary>>,
    pipelines: Mutex<HashMap<PipelineKey, MtlPipeline>>,
}
```
