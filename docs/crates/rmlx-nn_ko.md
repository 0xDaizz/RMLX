...
EP-2와 EP-4에서 추가된 두 모듈이 MoE 연산의 핵심 병목을 제거합니다.

**`expert_group.rs` (EP-2):** `ExpertGroup`은 여러 expert의 가중치를 단일 스태킹 텐서로 사전 결합하여, Gate -> Up -> fused SwiGLU -> Down 패스를 3개의 배치 GEMM으로 처리합니다. `GatherMM`(f16/bf16, `_into_cb` 지원)을 사용하여 expert별 개별 matmul 대신 단일 GPU 패스로 실행합니다.

**`moe_pipeline.rs` (EP-4):** `MoePipeline`은 TBO(Token-Before-Output) 및 SBO(Stage-Before-Output) 오버랩 전략을 `GpuEvent` signal/wait 체인으로 오케스트레이션합니다. 비행 중 CPU 대기를 제로로 줄이고, 단일 터미널 `GpuEvent::cpu_wait()`만 유지하여 compute와 RDMA 통신을 완전히 오버랩합니다.
