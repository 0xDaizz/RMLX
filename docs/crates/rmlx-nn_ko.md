...
Phase A는 프리필(seq_len=N) 단일 레이어 경로를 최적화하여 GPU 파이프라인을 디코드 이후로 확장합니다:

| 최적화 | 이전 | 이후 | 감소 |
|--------|------|------|------|
| CPU-GPU 동기화 지점 | 54 | 1 | 98.1% |
| SDPA 디스패치 (GQA) | 32 | 1 | 96.9% |
| 단일 레이어 속도 향상 | 1x | 3.5-7.3x | 시퀀스 길이 의존 |
| MLX 대비 (단일 레이어) | — | 1.2-3.4x 이내 | |
| GEMM TFLOPS | — | 24.05T (rmlx) vs 24T (MLX) | 파이프 패리티 달성 |

핵심 요소: 단일-CB 프리필 파이프라인, GQA slab SDPA, GEMM threadgroup swizzle, `matmul_into_cb`, `silu_into_cb`. 벤치마크: `prefill_bench.rs`, `gemm_bench.rs`.
...
| **융합 7-디스패치 (60L)** | **699.3 us/L** | **현재 최적** | **7 (융합)** |
| MLX 컴파일 | 4,525 (60L) | -- | -- |
| MLX 대비 격차 | | 6.34x 빠름 | |

M3 Ultra, f16, MoE expert 형상에서 벤치마크.

**Phase 11 결론:** 모든 커널 수준 최적화 실험(col-major GEMV, interleaved GEMV, SRAM prefetch + f16 누적 + function constant)이 699.3 us/layer 이하로 개선에 실패했습니다. Row-major BM8 GEMV + f32 누적이 73.6% 대역폭 효율을 달성하며, 이것이 Apple Silicon에서 f16 디코드의 실질적 한계입니다.
...
| 디코드 최적 | 699.3 us/layer | 7-디스패치 융합, 60L |
