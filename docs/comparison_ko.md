...
| **양자화** | Q4_0, Q4_1, Q8_0 | Q4_0, Q4_1, Q8_0, GGUF, AWQ, GPTQ | FP8, AWQ, GPTQ, INT4/INT8 |
| **Flash Attention** | 미지원 (fused SDPA만 지원) | 미지원 (fused SDPA) | FlashAttention-2/3 |
| **KV 캐시** | 정적 레이어별 캐시 | 정적 레이어별 캐시 | PagedAttention (vLLM) |
| **학습** | LoRA 파인튜닝만 지원 | 전체 학습 지원 | 전체 학습 + LoRA + QLoRA |
| **Op 모듈 수** | 14 | ~50+ | 수백 개 |
| **테스트 수** | 339+ | 광범위 | 광범위 |
| **완료된 Phase** | 0-9B-opt | N/A (안정 릴리스) | N/A (안정 릴리스) |
...
### 4.1 Flash Attention 미지원

RMLX는 fused SDPA(Scaled Dot-Product Attention)를 구현하고 있지만, tiled K/V와 online softmax를 갖춘 Flash Attention 2는 미지원입니다. Flash Attention은 어텐션 행렬의 메모리 사용량을 O(N^2)에서 O(N)으로 줄여 장문 컨텍스트 추론에 핵심적입니다.

MLX도 완전한 Flash Attention은 없지만, fused SDPA 구현이 더 성숙합니다.
...
MLX는 GGUF, AWQ, GPTQ 양자화 모델을 직접 로드할 수 있습니다. RMLX는 블록 양자화 Q4_0, Q4_1, Q8_0만 지원합니다.
...
### 5.1 Flash Attention 미지원

FlashAttention-2와 FlashAttention-3은 CUDA 생태계의 표준입니다. 메모리 효율(O(N^2) 대신 O(N))과 연산 효율(NVIDIA 하드웨어에서 더 나은 SRAM 활용) 모두를 제공합니다. RMLX의 fused SDPA는 긴 시퀀스에서 동등한 효율을 달성하지 못합니다.

### 5.2 Paged KV Cache 미지원

vLLM의 PagedAttention은 가상 메모리 페이지처럼 KV 캐시 메모리를 관리하여, 요청 간 효율적인 메모리 공유와 거의 제로에 가까운 낭비를 가능하게 합니다. RMLX는 정적 레이어별 KV 캐시 할당을 사용합니다.

### 5.3 Speculative Decoding 미지원

Speculative decoding (Medusa, EAGLE, draft-model 접근법)은 CUDA 서빙 스택의 표준입니다. 자기회귀 생성에서 2-3배 처리량 향상을 제공할 수 있습니다. RMLX는 어떤 형태의 speculative decoding도 구현하지 않습니다.

### 5.4 Continuous Batching 미지원

프로덕션 CUDA 서빙 엔진(vLLM, TensorRT-LLM, SGLang)은 continuous batching을 사용하여 배치에 요청을 동적으로 추가/제거함으로써 GPU 활용률을 극대화합니다. RMLX는 이를 지원하지 않습니다.
...
| **지연 시간** | 레이어당 6.8ms | 서브밀리초 재생 |
| **베이스라인 대비 속도 향상** | 16.15x | 일반적으로 2-5x (이미 효율적인 베이스라인 기준) |
| **구현 복잡도** | 중간 (결정론적 시퀀싱) | 낮음 (캡처 API가 직관적) |
| **메모리 오버헤드** | 최소 (재인코딩 시 버퍼 재사용) | 그래프 저장 (캡처된 연산) |

**핵심 인사이트**: ExecGraph의 16.15x 속도 향상은 CUDA의 기본 실행 모델보다 오버헤드가 큰 연산별 베이스라인 대비입니다. CUDA의 베이스라인은 스트림 순서 실행으로 이미 더 효율적이므로, CUDA Graphs는 더 강한 시작점에서 상대적으로 작은 개선을 제공합니다.
...
| GGUF (llama.cpp 형식) | 미지원 | 지원 | 지원 (네이티브 llama.cpp) |
| AWQ (activation-aware) | 미지원 | 지원 | 지원 (AutoAWQ) |
| GPTQ (post-training) | 미지원 | 지원 | 지원 (AutoGPTQ) |
| FP8 (E4M3/E5M2) | 미지원 | 부분 지원 | 지원 (H100+, 네이티브) |
...
RMLX의 양자화 범위는 기본 추론에 필요한 세 가지 블록 양자화 형식으로 제한됩니다. MLX와 CUDA 생태계는 훨씬 더 많은 형식을 지원하여, 더 낮은 비트 폭에서 더 나은 모델 품질을 달성할 수 있습니다.
...
이는 ExecGraph의 16.15x 성능 향상이 의미 있는 수치적 드리프트를 유발하지 않음을 보장합니다.

---

## 10. 동등성 달성 로드맵

RMLX는 4절과 5절에서 식별된 격차를 해소하기 위해 적극적으로 작업하고 있습니다. 계획된 Phase는 다음과 같습니다:

| Phase | 초점 | 주요 결과물 | 해소 대상 격차 |
|-------|------|------------|----------------|
| **Phase 10** | 어텐션 최적화 | Tiled K/V 기반 Flash Attention 2, Paged KV Cache | 4.1절, 5.1절, 5.2절 |
| **Phase 11** | 서빙 최적화 | Continuous batching, speculative decoding, 동적 스케줄링 | 5.3절, 5.4절 |
| **Phase 12** | 고급 양자화 | GGUF 로더, AWQ, GPTQ, FP8 지원 | 4.4절, 8절 |

이 Phase들은 아직 일정이 확정되지 않았습니다. 현재 초점(Phase 0-9B-opt)은 Metal GPU 파이프라인과 ExecGraph 최적화에 있습니다.

---

## 요약

RMLX는 MLX나 CUDA의 대체품이 아닙니다. 특정 니치를 차지합니다: **Apple Silicon에서의 고성능 Metal GPU 추론, Rust로 작성, Thunderbolt 5 RDMA 기반 분산 실행에 최적화**.

**RMLX를 선택해야 할 때**: Apple Silicon 하드웨어에서 최대 추론 성능이 필요할 때, Python 의존성 없는 단일 Rust 바이너리를 원할 때, 또는 Mac Studio 분산 추론 클러스터를 구축할 때.
