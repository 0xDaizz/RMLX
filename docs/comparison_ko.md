...
| **CB 관리** | ExecGraph + 9-디스패치: 65 CBs/layer -> 9개 디스패치를 가진 1 CB (연산별 베이스라인 대비 64x, MLX 대비 2.09x 빠름 (60L)) | eval() 배치당 1 CB | Stream 기반, CUDA Graphs capture-replay |
...
| **Op 모듈 수** | 27 | ~50+ | 수백 개 |
| **테스트 수** | 1,142+ | 광범위 | 광범위 |
| **완료된 Phase** | 0-9B-opt + S1-S5 + Phase KO + Phase 8c | N/A (안정 릴리스) | N/A (안정 릴리스) |
...
| MLX compiled 대비 격차 | 약 4.8배 느림 | 2.09x 빠름 (60L) |
| 레이턴시 (Cached 2-enc, 60L) | — | 1,367 us/L (8% 빠름, 6x 낮은 σ) |
...
| **지연 시간** | 레이어당 ~1.2-1.4ms (9-디스패치 / cached 2-인코더 경로) | 서브밀리초 재생 |
