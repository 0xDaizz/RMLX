...
| Fused 7-디스패치 (60L) | -- | 699.3 us/L | 7-dispatch fused 최적 |
...
Dense 또는 MoE transformer 모델의 단일 transformer 레이어는 약 65개의 개별 연산을 실행합니다. 각 연산은 command buffer 생성, 인코더 설정, CPU-GPU 동기화 포인트에 대한 CPU 측 오버헤드를 발생시킵니다. 레이어당 65개의 command buffer가 있으면, CPU가 GPU 작업을 관리하는 데 GPU가 실행하는 것보다 더 많은 시간을 소비합니다.
...
fn forward(&self, input: &Array) -> Result<Array> {
    ...
    // ... pass.set_buffer(), pass.dispatch_1d() 등으로 작업 인코딩 ...
    pass.end();
    cb.commit();
...
단일 transformer 레이어(4096 hidden, M3 Ultra)에서 측정한 벤치마크입니다:
...
- **실측**: 699.3 us/layer (f16, 60L, M3 Ultra)
- **감소**: 9 디스패치 → 7 디스패치 (GPU 디스패치 22% 감소)
- **개선**: 714 us/L → 699.3 us/L (커널 융합으로 2.1% 레이턴시 감소)

---

## Phase 11: GEMV 커널 최적화 실험 — 종결

Phase 11은 Phase 10에서 확립한 699.3 us/layer 하한선 이하로 성능을 끌어내리기 위해 세 가지 대안적 GEMV 커널 전략을 조사했습니다. 세 가지 실험 모두 성능 개선에 실패하여, 현재의 row-major BM8 GEMV + f32 누산기가 699.3 us/layer의 실질적 하한선임을 확인했습니다.
...
디코드 최적화는 커널 수준에서 **종결**되었습니다. 699.3 us/layer (fused 7-dispatch)가 현재 Apple Silicon에서 f16 디코드의 최적 레이턴시입니다.
...
단일 transformer 레이어(f16, M3 Ultra)에서 측정:
...
GEMM 처리량 격차는 Phase B config sweep, Phase C 커널 수준 최적화, Phase D2 MLX 아키텍처 커널을 통해 13T에서 24.05T TFLOPS로 해소되었습니다 (MLX: 23.97T, pipe parity). 프로덕션 커널에서 wide_load (2×half4), SG=2×4 레이아웃, serpentine MMA를 적용합니다.
