...
| 융합 7-디스패치 디코드 (60L) | -- | 703.4 us/L | MLX 대비 6.43x 빠름 |
| 열-주도 (실험적) | -- | 핫 패스에서 미사용 | Phase 11 참고 |
...
- **최대 절대 차이:** 6.4e-6
- **검증:** 1,356 전체 테스트가 양쪽 코드 경로에서 통과
...
- **달성**: 703.4 us/layer (f16, 60L, M3 Ultra) — MLX 대비 6.43x 빠름
- **감소**: 9 디스패치 → 7 디스패치 (GPU 디스패치 22% 감소)

## Phase 11: 열-주도(Column-Major) GEMV 디코드 경로

Phase 11은 가중치를 열-주도 [K,M] 레이아웃으로 저장하여 연속 half4 로드를 통해 GEMV 대역폭을 최적화하고, 레지스터 수준 프리페치로 메모리 코얼레싱을 개선합니다.

### 열-주도 최적화

**열-주도 가중치 저장**: 기존 행-주도 GEMV는 K 차원을 따라 누적할 때 비연속 요소를 읽습니다. 가중치를 [K,M] 열-주도 레이아웃으로 전치하면 각 스레드가 연속된 half4 (8바이트) 청크를 읽어 메모리 대역폭 활용을 극대화합니다.

**레지스터 수준 프리페치**: 현재 반복의 연산 중에 다음 반복의 가중치 데이터를 레지스터에 로드하여 메모리 레이턴시를 연산 뒤에 숨깁니다.

### 새 커널 (총 12개)

`gemv.rs`의 열-주도 커널 6개: `gemv_col`, `gemv_bias_col` (f32/f16/bf16)
`fused.rs`의 열-주도 커널 6개: `fused_rms_gemv_col`, `fused_swiglu_down_col` (f32/f16/bf16)

### 통합

- `Linear`의 `prepare_weight_col_major()`가 초기화 시 가중치를 [K,M] 열-주도 레이아웃으로 전치
- `CachedDecode`가 열-주도 PSO와 가중치 버퍼를 통합 (레이어당 4개 열-주도 가중치 버퍼)
- 열-주도 PSO 컴파일 실패 시 행-주도로 자동 폴백

### 성능 결과

Phase 11은 모든 BM8 변형(gemv, gemv_bias, fused_rms_gemv, fused_swiglu_down x f32/f16/bf16)에 대해 12개의 열-주도 GEMV Metal 커널을 추가합니다. 열-주도 저장은 TM=4 행에 대해 연속 half4 로드를 가능하게 하지만, 벤치마킹 결과 k-위치 간의 stride-M 접근 패턴이 LLM 가중치 행렬에서 일반적인 대형 M 차원(M>=4096)에서 성능을 저하시키는 것으로 확인되었습니다. 열-주도 커널은 향후 소형 M 사용 사례를 위해 유지되지만 디코드 핫 패스에서는 사용되지 않습니다. 성능은 Phase 10 수준을 유지합니다 (703.4 us/layer, MLX 대비 6.43x 빠름).
