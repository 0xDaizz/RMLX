...
| **핵심 모듈** | `device.rs` (디바이스 + 아키텍처 감지), `queue.rs` (듀얼 큐 관리), `command.rs` (CommandBuffer/Encoder), `event.rs` (SharedEvent), `pipeline.rs` (PSO 캐시) |
| **의존성** | metal-rs 0.31, objc2, block2 |
| **현재 상태** | Phase 0 완료 — `GpuDevice`, `StreamManager`, `DeviceStream` 기본 구조 구현 |
...
| **핵심 모듈** | `zero_copy.rs` (ZeroCopyBuffer, CompletionFence), `pool.rs` (이중 등록 버퍼 풀), `cache.rs` (size-binned 캐시), `stats.rs` (할당 통계) |
| **의존성** | `rmlx-metal`, libc |
| **현재 상태** | Phase 0 완료 — ZeroCopyBuffer 기본 구조 및 in-flight 참조 카운팅 구현 |
...
| **핵심 모듈** | `ffi.rs` (ibverbs 바인딩), `context.rs` (ibv_context/PD/CQ), `qp.rs` (UC QP), `mr.rs` (메모리 영역 등록), `collective.rs` (all-to-all, allreduce), `multi_port.rs` (포트 스트라이핑) |
| **의존성** | `rmlx-alloc`, libc (ibverbs FFI) |
| **현재 상태** | Phase 0 스켈레톤 — FFI 바인딩 구조 정의, `build.rs` ibverbs 링크 설정 |
...
| **핵심 모듈** | `dtype.rs` (f32, f16, bf16, quantized), `array.rs` (N-dim array), `ops/` (matmul, softmax 등 연산), `kernels/` (AOT/JIT 커널 관리), `graph.rs` (연산 그래프), `scheduler.rs` (스트림별 스케줄러) |
| **의존성** | `rmlx-metal`, `rmlx-alloc` |
| **현재 상태** | Phase 0 스켈레톤 — dtype, array 기본 타입 정의 |
...
| **핵심 모듈** | `group.rs` (rank/world_size 추상화), `primitives.rs` (AllReduce, AllGather 등), `moe/` (3-zone auto backend, MoE dispatch/combine), `pipeline.rs` (compute-RDMA 오버랩) |
| **의존성** | `rmlx-core`, `rmlx-rdma` |
| **현재 상태** | Phase 0 스켈레톤 — 모듈 구조 정의 |
...
| **핵심 모듈** | `linear.rs` (quantized Linear), `attention.rs` (Multi-head/GQA), `transformer.rs` (Transformer block), `moe.rs` (gate + routing), `models/` (LLaMA, Qwen, DeepSeek-V3, Mixtral) |
| **의존성** | `rmlx-core` |
| **현재 상태** | Phase 0 스켈레톤 — 모듈 구조 정의 |
