...
>
> 상태: 전 Phase 완료 (0-11 + S1-S5 + KO + 8c + A-D + F-J + objc2-metal 마이그레이션) (1,298+ tests, 0 failures) | 라이선스: MIT | Rust 1.80+ | macOS (Apple Silicon)
...
디코드: 699.3 us/layer (fused 7-dispatch, 64x 속도 향상, MLX 대비 6.34x 빠름 (60L))
BW 효율: 73.6% — 커널 수준 최적화 종결

GEMM: 24.05T TFLOPS (MLX 23.97T 대비 pipe parity)
QMM Q4 M=512: 17.43T (MLX 13.6T 대비 +28%)
QMM Q4 low-M: 86.3% parity
프리필 GPU-only: 29.2T
Fair bench M=32~256: MLX 대비 1.24~2.83x 빠름
```
...
6. **ExecGraph CB 배칭**
   Fused 7-dispatch 파이프라인이 60L 기준 699.3 us/layer 달성 (64x 속도 향상,
   MLX 대비 6.34x 빠름). 73.6% 대역폭 효율 — f16 디코드의 실질적 하한선.

7. **프리필 최적화**
   단일 CB 파이프라인(54개 동기화 지점 → 1개), GQA slab SDPA(32개 per-head 디스패치 → 1개). GEMM 처리량 24.05T (MLX와 pipe parity). 프리필 GPU-only 처리량: 29.2T. 베이스라인 대비 3.5-7.3x 속도 향상.
