...
//!
//! Run with:
//!   cargo bench -p rmlx-core --bench gemm_sweep2
...
const CONFIGS: &[GemmConfig] = &[
    GemmConfig { label: "bk8_2x2",   bm: 64, bn: 64, bk: 8,  sg_rows: 2, sg_cols: 2, double_buffered: true },
    GemmConfig { label: "bk16_4x2",  bm: 64, bn: 64, bk: 16, sg_rows: 4, sg_cols: 2, double_buffered: true },
    GemmConfig { label: "bk16_2x4",  bm: 64, bn: 64, bk: 16, sg_rows: 2, sg_cols: 4, double_buffered: true },
    GemmConfig { label: "bk32_2x2",  bm: 64, bn: 64, bk: 32, sg_rows: 2, sg_cols: 2, double_buffered: true },
    GemmConfig { label: "bk32_2x4",  bm: 64, bn: 64, bk: 32, sg_rows: 2, sg_cols: 4, double_buffered: true },
    GemmConfig { label: "bk32_1x2",  bm: 64, bn: 64, bk: 32, sg_rows: 1, sg_cols: 2, double_buffered: true },
    GemmConfig { label: "bk16_4x4",  bm: 64, bn: 64, bk: 16, sg_rows: 4, sg_cols: 4, double_buffered: true },
    GemmConfig { label: "bk32_4x2",  bm: 64, bn: 64, bk: 32, sg_rows: 4, sg_cols: 2, double_buffered: true },
    GemmConfig { label: "bk16_2x2",  bm: 64, bn: 64, bk: 16, sg_rows: 2, sg_cols: 2, double_buffered: true },
];
...
fn generate_gemm_shader(cfg: &GemmConfig) -> String {
    ...
    s.push_str(&format!(
        "kernel void gemm_sweep2_{label}(\n\
    ...
    // All configs are double-buffered
...
fn bench_config(
    ...
    let kernel_name = format!("gemm_sweep2_{}", cfg.label);
    ...
    let tg = MTLSize::new(cfg.n_threads() as u64, 1, 1);
...
fn main() {
    ...
    for cfg in CONFIGS {
        ...
        let jit_name = format!("gemm_sweep2_{}", cfg.label);
        registry
            .register_jit_source(&jit_name, &source)
            .unwrap_or_else(|e| panic!("Failed to compile {}: {e}", cfg.label));
    }

    println!("=== GEMM Sweep2 Benchmark (f16) ===");
    println!(
        "Variants: {}, M values: 11, N values: [4096, 14336]",
        CONFIGS.len()
    );
    println!("Warmup: {}, Bench: {}", WARMUP_ITERS, BENCH_ITERS);
    println!();

    let m_values = [64, 96, 128, 192, 256, 384, 512, 768, 1024, 1536, 2048];
