...
// ---------------------------------------------------------------------------
// Encode into existing command buffer (no commit/wait)
// ---------------------------------------------------------------------------

/// Encode a binary operation into an existing command buffer (no commit/wait).
///
/// **Caller must ensure both inputs are contiguous** and dtypes match.
/// Supports same-shape Flat dispatch and scalar broadcast variants.
/// General N-dim broadcast is also supported.
...
/// Element-wise addition into existing command buffer (no commit/wait).
pub fn add_into_cb(
    registry: &KernelRegistry,
    a: &Array,
    b: &Array,
...
/// Element-wise multiplication into existing command buffer (no commit/wait).
pub fn mul_into_cb(
    registry: &KernelRegistry,
    a: &Array,
    b: &Array,
